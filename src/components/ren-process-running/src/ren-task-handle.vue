  ...
  methods: {
    ...
    rejectTask: debounce(function () {
      this.$refs['dataForm'].validate((valid) => {
        ...
        this.$http['post']('/api/act/task/backToFirst?', params).then(({ data: res }) => {
          if (res.code !== 0) {
    ...
    completeTask: debounce(function () {
      this.$refs['dataForm'].validate((valid) => {
        ...
        this.$http.post('/api/act/task/complete?' + params).then(({ data: res }) => {
          if (res.code !== 0) {
            this.$message.error(res.msg)
            if (this.callbacks.taskHandleErrorCallback) {
              this.callbacks.taskHandleErrorCallback(res)
    ...
    terminationTask: debounce(function () {
      this.$refs['dataForm'].validate((valid) => {
        ...
        this.$http['post']('/api/act/task/endProcess', task).then(({ data: res }) => {
          if (res.code !== 0) {
            this.$message.error(res.msg)
            if (this.callbacks.taskHandleErrorCallback) {
              this.callbacks.taskHandleErrorCallback(res)
