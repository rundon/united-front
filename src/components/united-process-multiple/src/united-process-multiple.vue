<template>
  <div>
    <united-process-start v-if="startProcessVisible" :saveFormUrl="saveFormUrl" :updateInstanceIdUrl="updateInstanceIdUrl" :dataFormName="dataFormName" ref="renProcessStart"></united-process-start>
    <united-process-running v-if="runningHandleVisible" ref="renProcessRunning"></united-process-running>
    <united-process-detail v-if="processDetailVisible" ref="renProcessDetail"></united-process-detail>
  </div>
</template>

<script>
export default {
  name: 'UnitedProcessMultiple',
  data () {
    return {
      // 是否显示启动流程
