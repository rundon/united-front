  ...
  data () {
    return {
      mixinViewModuleOptions: {
        getDataListURL: '/api/schedule/page',
        getDataListIsPage: true,
        deleteURL: '/api/schedule',
        deleteIsBatch: true
      },
      dataForm: {
        beanName: ''
  ...
  methods: {
    ...
    pauseHandle (id) {
      ...
      this.$confirm(this.$t('prompt.info', { 'handle': this.$t('schedule.pause') }), this.$t('prompt.title'), {
        ...
        this.$http.put('/api/schedule/pause', id ? [id] : this.dataListSelections.map(item => item.id)).then(({ data: res }) => {
          if (res.code !== 0) {
    ...
    resumeHandle (id) {
      ...
      this.$confirm(this.$t('prompt.info', { 'handle': this.$t('schedule.resume') }), this.$t('prompt.title'), {
        ...
        this.$http.put('/api/schedule/resume', id ? [id] : this.dataListSelections.map(item => item.id)).then(({ data: res }) => {
          if (res.code !== 0) {
            return this.$message.error(res.msg)
          }
          this.$message({
    ...
    runHandle (id) {
      ...
      this.$confirm(this.$t('prompt.info', { 'handle': this.$t('schedule.run') }), this.$t('prompt.title'), {
        ...
        this.$http.put('/api/schedule/run', id ? [id] : this.dataListSelections.map(item => item.id)).then(({ data: res }) => {
          if (res.code !== 0) {
            return this.$message.error(res.msg)
          }
          this.$message({
