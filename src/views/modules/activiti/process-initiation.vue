...
export default {
  ...
  data () {
    return {
      mixinViewModuleOptions: {
        getDataListURL: '/api/act/process/lastestPage',
  ...
  methods: {
    ...
    createProcessInstance (row) {
      this.$http.get(`/api/act/process/getProcDefBizRoute/${row.id}`).then(({ data: res }) => {
        if (res.code !== 0) {
    ...
    startProcess (key) {
      ...
      this.$http.post(`/api/act/running/start`, params).then(({ data: res }) => {
        if (res.code !== 0) {
          return this.$message.error(res.msg)
        }
        this.$message({
