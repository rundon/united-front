...
export default {
  ...
  data () {
    return {
      mixinViewModuleOptions: {
        getDataListURL: '/api/act/process/page',
        getDataListIsPage: true,
        deleteURL: '/api/act/process',
        deleteIsBatch: true,
  ...
  methods: {
    ...
    getResourceURL (id, name) {
      ...
      return `${window.SITE_CONFIG['apiURL']}/api/act/process/resource?${params}`
    },
    // 部署流程文件
    deployHandle () {
    ...
    activeHandle (id) {
      this.$confirm(this.$t('prompt.info', { 'handle': this.$t('process.active') }), this.$t('prompt.title'), {
        ...
        this.$http.put(`/api/act/process/active/${id}`).then(({ data: res }) => {
          if (res.code !== 0) {
    ...
    suspendHandle (id) {
      this.$confirm(this.$t('prompt.info', { 'handle': this.$t('process.suspend') }), this.$t('prompt.title'), {
        ...
        this.$http.put(`/api/act/process/suspend/${id}`).then(({ data: res }) => {
          if (res.code !== 0) {
            return this.$message.error(res.msg)
          }
          this.$message({
    ...
    convertToModelHandle (id) {
      this.$confirm(this.$t('prompt.info', { 'handle': this.$t('process.convertToModel') }), this.$t('prompt.title'), {
        ...
        this.$http.post(`/api/act/process/convertToModel/${id}`).then(({ data: res }) => {
          if (res.code !== 0) {
            return this.$message.error(res.msg)
          }
          this.$message({
