...
export default {
  ...
  methods: {
    ...
    // 获取角色列表
    getRoleList () {
      return this.$http.get('/api/sys/role/list').then(({ data: res }) => {
        if (res.code !== 0) {
    ...
    // 获取信息
    getInfo () {
      this.$http.get(`/api/sys/user/${this.dataForm.id}`).then(({ data: res }) => {
        if (res.code !== 0) {
          return this.$message.error(res.msg)
        }
        this.dataForm = {
    ...
    dataFormSubmitHandle: debounce(function () {
      this.$refs['dataForm'].validate((valid) => {
        ...
        this.$http[!this.dataForm.id ? 'post' : 'put']('/api/sys/user', {
          ...this.dataForm,
          roleIdList: [
