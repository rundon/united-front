...
export default {
  ...
  methods: {
    ...
    // 获取菜单列表
    getMenuList () {
      return this.$http.get('/api/sys/menu/select').then(({ data: res }) => {
        if (res.code !== 0) {
    ...
    // 获取信息
    getInfo () {
      this.$http.get(`/api/sys/role/${this.dataForm.id}`).then(({ data: res }) => {
        if (res.code !== 0) {
          return this.$message.error(res.msg)
        }
        this.dataForm = {
    ...
    dataFormSubmitHandle: debounce(function () {
      this.$refs['dataForm'].validate((valid) => {
        ...
        this.$http[!this.dataForm.id ? 'post' : 'put']('/api/sys/role', this.dataForm).then(({ data: res }) => {
          if (res.code !== 0) {
            return this.$message.error(res.msg)
          }
          this.$message({
