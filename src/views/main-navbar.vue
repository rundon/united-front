...
export default {
  ...
  created () {
    var vue = this
    socket = new WebSocket(`${window.SITE_CONFIG['socketURL']}?access_token=`+this.$store.state.token)
  ...
  methods: {
    ...
    getUnReadCount () {
      this.$http.get(`/api/notice/mynotice/unread`).then(({ data: res }) => {
        if (res.code !== 0) {
    ...
    logoutHandle () {
      this.$confirm(this.$t('prompt.info', { 'handle': this.$t('logout') }), this.$t('prompt.title'), {
        ...
        this.$http.get('/oauth/logout').then(({ data: res }) => {
          if (res.code !== 0) {
            return this.$message.error(res.msg)
          }
          clearLoginInfo()
